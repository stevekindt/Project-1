...
$("#enterCity").on('click',function(event){
    ...
    var queryURL = "https://api.openweathermap.org/data/2.5/weather?q=" + city +"&units=imperial" + weatherAPIKey;
    ...
    $.ajax({
        url: "https://api.openweathermap.org/data/2.5/forecast?q=" + city + "&units=imperial" + weatherAPIKey,
        method: "GET"
    }).then(function (response) {
    ...
    $.ajax({
        url: "https://api.openweathermap.org/data/2.5/uvi?lat=" + response.city.coord.lat + "&lon=" + response.city.coord.lon + weatherAPIKey,
        method: "GET"
    }).then(function (data) {
        console.log(data);
